...
import { authLoginSchema, checkEmailSchema } from "../c_auth/auth.schema";
...
const getUserByEmail = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const { value } = checkEmailSchema.validate(req.query);
...
const createUser = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const { name, email, password_hash } = value
...
const updateUserById = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const { value } = checkEmailSchema.validate(req.body);

    const email = value.email;
    logger.info(`Probando correo ${email}`);
    const id_user = req.user;
